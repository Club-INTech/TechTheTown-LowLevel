...
//    #endif                        \*m'voyez
    char orderBufferWithCharID[RX_BUFFER_SIZE];
    strcpy(orderBufferWithCharID, orderToExecute);
    highLevel.acknowledge(orderBufferWithCharID);

    if (orderBufferWithCharID[0]!=charIDLastMessage) {
        charIDLastMessage=orderBufferWithCharID[0];
        char* orderBuffer = orderBufferWithCharID+1;
                ...
                */
                ...
                if (motionControlSystem.translation)
                    ...
                    highLevel.log("reglage de la rotation");
                               ...
                               "display")) //affiche les paramètres des PID des différentes asserv (translation, rotation, vitesse à droite, vitesse à gauche)
            {
                float
                ...
                */
                ...
                if (n_param == 2) {
                    actuatorsMgr.movAX12G((unsigned int) parseInt(orderData.at(1)),
                                          (uint16_t) parseInt(orderData.at(2)));
                } else {
                    highLevel.log("ERREUR::Paramètres incorrects");
                }
                ...
                if (n_param == 2) {
                    actuatorsMgr.setAX12GSpeed((unsigned int) parseInt(orderData.at(1)),
                                               (uint16_t) parseInt(orderData.at(2)));
                } else {
                    highLevel.log("ERREUR::Paramètres incorrects");
                }
